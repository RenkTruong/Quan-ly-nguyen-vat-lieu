        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full mt-1.5 w-64 bg-slate-800 border border-slate-700 rounded-xl shadow-xl py-2 hidden group-hover:block z-50">
                <div className="px-3 py-1 text-[11px] font-medium text-slate-400 uppercase tracking-wider border-b border-slate-700 mb-1">
                  Đổi vai trò để test phân quyền:
                </div>
                {allUserRoles.map((ur) => (
                  <button
                    key={ur.email}
                    onClick={() => onSelectUserRole(ur)}
                    className={`w-full text-left px-3 py-2 text-xs flex flex-col transition-colors ${
                      ur.email === currentUserRole.email
                        ? 'bg-slate-700/80 text-emerald-400 font-semibold'
                        : 'text-slate-200 hover:bg-slate-700/50'
                    }`}
                  >
                    <span>{ur.fullName}</span>
